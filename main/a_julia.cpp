...
RGBColor a1computeColor_for_bonus(int x, int y, int width, int height, RGBColor c) {
    float fx =  float(x) / float(width);
    float fy =  float(y) / float(height);
    RGBColor color = RGBColor::rep(0.0f);
    color = color + a1computeWeight(fx, fy, Point(-0.8f, 0.156f, 0.0f), 64.0f) * c;
    color = color + a1computeWeight(fx, fy, Point(-0.6f, 0.2f, 0.0f), 64.0f)*0.2f * c;
    color = color + a1computeWeight(fy, fx, Point(0.285f, 0.0f, 0.0f), 64.0f) * c;
    return RGBColor::rep(1.0f) - color;
}
...
void a_julia() {
    ...
    engine.test_render_bonus(img);
    img.writePNG("a5-bonus.png");
}